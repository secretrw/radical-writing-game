...
let turnPhase = 'waiting'; // waiting | running | paused | timeUp
...
io.on('connection', (socket) => {
  ...
  socket.on('teacherNextTurn', (wordWritten) => {
    ...
    if (typeof wordWritten === 'string' && wordWritten.trim() !== "" && !usedWords.includes(wordWritten)) {
    ...
    setTimeout(() => {
      ...
      // [修改] 勝者判定：只剩一位玩家時（包含一開始就只有一人且答對）宣布冠軍
